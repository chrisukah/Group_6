...
library(formattable)
subset_df3_new_missing <- df3_new_missing %>% select(age_category, sex, cumulative_fully_vaccinated, cumulative_up_to_date_vax, yr_dt_vax)

formattable(head(subset_df3_new_missing, 10), list(
  col.names = gsub("_", " ", names(subset_df3_new_missing)),
  caption = "Portion of Summary Statistic of simulated data set from California and vaccination rates of COVID 19"
))
describe(subset_df3_new_missing)
...
```{r include=FALSE}
library(dplyr)
#kable(subset_df3_new_missing, booktabs = T, caption = "California quarterly COVID vaccination #rates by year, age group and sex in different counties") %>% group_rows("0-17years", 1, 10)
  #footnote(general = "California  simulated morbidity flu data",
...
#join_flu_vax <- left_join(final_flu, df3_new_missing, by = "age_category")
