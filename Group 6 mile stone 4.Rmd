...
```{r}
#Import data on vaccination rate
df_vax<-read.csv("https://raw.githubusercontent.com/PHW290/phw251_projectdata/main/scenario_1/ca_vax_rates_quarter.csv")
```

```{r}
#Study dataset on vaccination rate
str(df_vax)
```


```{r}
#HARMONISE COLUMN NAMES IN THE df_vax Dataframe
# Get the current column names
current_colnames <- colnames(df_vax)
...
colnames(df_vax) <- new_colnames
head(df_vax)
```

```{r}
# Rename similar columns in the two datasets to have same names
df_vax1 <- df_vax %>%
  rename(
county = county_name,
age_group = demographic_value,)
```
