...
ca_vax1 <- df_ca_vax_rates %>% 
  rename("county" = "County Name", "age_category" = "Demographic Value") %>%
  filter(age_category == "Age Group") %>% 
  na.omit()

final_flu <- bind_rows(df1, df2_1)

join_flu_vax <- left_join(final_flu, ca_vax1, by = c("county", "age_category"))
```
